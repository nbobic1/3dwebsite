  ...
  
{
    ...
    func: () => {
        ...
        if(scene.children.length==pos+1)
        {
         ...
         scene.children[pos].rotation.y=lerp(rot_val, 1.5*Math.PI, scalePercent(animationScripts[0].start, animationScripts[0].end));  
...
},
{
    ...
    func: () => {
        if(scene.children.length==pos+1)
        {
         scene.children[pos].position.x=lerp(30, -20, scalePercent(animationScripts[1].start, animationScripts[1].end));
         scene.children[pos].rotation.y=lerp(1.5*Math.PI, -Math.PI/2, scalePercent(animationScripts[1].start,animationScripts[1].end));  
         scene.children[pos].scale.y=lerp(0.5, 0, 3*scalePercent(animationScripts[1].start, animationScripts[1].end)/4);  
         scene.children[pos].scale.x=lerp(0.5, 0, 3*scalePercent(animationScripts[1].start,animationScripts[1].end)/4);  
         scene.children[pos].scale.z=lerp(0.5, 0, 3*scalePercent(animationScripts[1].start, animationScripts[1].end)/4);  
        }   
...
const observer = new IntersectionObserver(entries => {
 ...
 observer.observe(document.querySelector('.square'));
  ...
  function moveJoint(mouse,  degreeLimit) {console.log("bruhh");
